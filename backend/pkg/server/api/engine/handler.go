...
func newEngineHandler(cl cluster.Cluster, config APIConfiguration) (gin.HandlerFunc, error) {
	...
	return tonic.Handler(func(ctx *gin.Context, in *EngineRequest) (*EngineResponse, error) {
		...
		client, err := cl.ClientInterface(clusterResources...)
		if err != nil {
			return nil, err
		}
		processor, err := engine.NewProcessor(params, config, client, dClient, cmResolver, cl.PolicyExceptionSelector(exceptions))
		if err != nil {
			return nil, err
		}
